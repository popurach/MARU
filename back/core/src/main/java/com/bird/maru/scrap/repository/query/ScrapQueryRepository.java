...
public interface ScrapQueryRepository extends JpaRepository<Scrap, ScrapId> {
    ...
    @Query("SELECT s FROM Scrap s WHERE s.member.id = :memberId AND s.spot.id = :spotId")
    Optional<Scrap> findByMemberAndSpot(@Param("memberId") Long memberId, @Param("spotId") Long spotId);
